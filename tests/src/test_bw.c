...
test_ca_bw_pois_e_step (void)
{
    ...
    scalar eq[] = {13, 14, 8, 10, 16, 26, 32, 27, 18, 32, 36, 24, 22, 23, 22, 18, 25, 21, 21,
...
test_update_lambda (void)
{
    ...
    DataSet inp = { eq, 107 };
    HmmProbs *probs = ca_NewHmmProbs (inp.size, hmm.m_states);
    scalar *cbuffer = alloc_block_fill (hmm.m_states * inp.size, 0);

    ca_bw_pois_e_step (&inp, &hmm, probs);
    update_lambda (&inp, probs->lalpha, probs->lbeta, hmm.m_states, hmm.llh, cbuffer,
                hmm.params->lambda);

    puts("\n");
    for (size_t i = 0; i< hmm.m_states; i++)
    {
        printf ("L_%zu_b: %Lf\tL_%zu_a: %Lf\n", i, hmm.init->lambda[i], i, hmm.params->lambda[i]);
    }
    PoisHmm_FreeParams (hmm.init);
    PoisHmm_FreeParams (hmm.params);
    ca_FreeHmmProbs (probs);
    return false;
}
